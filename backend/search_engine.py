...
class SearchEngine:
    ...
    def search(self, query: str, top_k: int = 5, binary_context: str = None):
        ...
        for name, col in collections.items():
            try:
                # Apply filter for binary_analysis_findings and binary_functions to only show relevant binary info
                where_filter = None
                if binary_context and name in ["binary_analysis_findings", "binary_functions"]:
